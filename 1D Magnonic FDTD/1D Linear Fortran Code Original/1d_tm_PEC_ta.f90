	...
	REAL*8 MXN(1:NY+1), MYN(1:NY+1) !magnetization step n
	REAL*8 HEX_X(1:NY+1), HEX_Y(1:NY+1) !excahnge field
	REAL*8 oldMXN(1:NY+1), oldMYN(1:NY+1) !magnetization step n, previous iteration
...
! MAX VALUE OF K PARAMETER
    ...
    H0 = 4000.0*OE*(1.0D0 - DBLE(IH0-1)/DBLE(NH0))
    !H0 = 450.0D0*OE
...
! UPDATE THE BX VALUES -- STEP 1
		...
		DO !iterations
			...
			DO J=J1, J2
				...
				X0_X = MX(J) + ALPHA*MY(J) + 0.5D0*MS*DABS(GAMMA)*DT*(HYN(J) + HEX_Y(J))
				X0_Y = MY(J) - ALPHA*MX(J) - 0.5D0*MS*DABS(GAMMA)*DT*(HXN(J) + HEX_X(J))
								
				AZ = -0.5D0*DABS(GAMMA)*DT*H0 - ALPHA
				
				oldMXN(J) = MXN(J) !previous iteration
				oldMYN(J) = MYN(J) !previous iteration
							
				MXN(J) = (X0_X + AZ*X0_Y)/(1.0D0 + AZ**2) !step n, current iteration
				MYN(J) = (X0_Y - AZ*X0_X)/(1.0D0 + AZ**2) !step n, current iteration				
			...
			DO J=J1, J2
				...
				TMP1 = TMP1 + DSQRT(MXN(J)**2+MYN(J)**2)
				TMP2 = TMP2 + DSQRT(oldMXN(J)**2+oldMYN(J)**2)	
